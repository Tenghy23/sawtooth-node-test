...
const validatorEndpoint = 'tcp://sawtooth-validator-default:4004';
...
function sendBatchListToValidator(batchList) {
  return new Promise(async (resolve, reject) => {
    const socket = new zmq.Push();

    try {
      await socket.connect(validatorEndpoint);

      // Send the encoded BatchList
      await socket.send(batchList);
      console.log('BatchList sent to the validator.');
